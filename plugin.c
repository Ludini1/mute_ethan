...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Mute Ethan";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Mute Ethan";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "This plugin mutes Ethan.";
}
...
enum {
	...
	MENU_CLIENT_TAG_FOR_MUTE,
	MENU_CLIENT_UNTAG_FOR_MUTE
};
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_CLIENT_TAG_FOR_MUTE, "Tag for mute", "");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_CLIENT_UNTAG_FOR_MUTE, "Untag for mute", "");
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	...
	switch(type) {
		...
		case PLUGIN_MENU_TYPE_CLIENT:
			...
			switch(menuItemID) {
				case MENU_CLIENT_TAG_FOR_MUTE:
					/* Menu client 1 was triggered */
					muteId[0] = selectedItemID;
					break;
				case MENU_CLIENT_UNTAG_FOR_MUTE:
...
void ts3plugin_onHotkeyEvent(const char* keyword) {
	if (!strcmp(keyword, "mute")) {
		if (isActive) {
			isMuted =! isMuted; // Toggle Client Mute
			
			if (isMuted) {
				ts3Functions.requestMuteClients(ts3Functions.getCurrentServerConnectionHandlerID(), muteId, NULL);
			} else {
				ts3Functions.requestUnmuteClients(ts3Functions.getCurrentServerConnectionHandlerID(), muteId, NULL);
			}
		}
	}
}
